...
const Hero = () => {
  return (
    <section
      id="home"
      className="w-full lg:h-screen flex flex-col items-center justify-center  lg:flex-row px-4 sm:px-8 md:px-16 relative overflow-hidden"
    >
      ...
      <div className="w-full max-w-xl flex items-center justify-center relative mt-4 lg:mt-20 flex-col">
        <div className="relative flex items-center justify-center w-[19rem] h-[22rem] sm:w-[25rem] sm:h-[31.5rem] md:w-[31rem] md:h-[38rem] lg:w-[42rem] lg:h-[49rem] pb-18">
          <img
            src={bgBack}
            alt="backImage"
          ...
          <img
            src={bgFront}
            alt="frontImage"
            className="absolute -translate-x-1/2 -translate-y-[45%] left-[51%] top-[42%] sm:top-[47%] w-[19rem] h-[20rem] sm:w-[22rem] sm:h-[28.5rem] md:w-[38rem] md:h-[44rem] object-contain drop-shadow-xl z-50"
          />
        </div>
      </div>

      {/* Arrow Down: show below images on Desktop/mobile/tablet */}
      <Link
        to="about-us"
        className="w-10 h-10 border-1 border-[var(--color-secondary)] bg-[var(--color-secondary-transparent)] flex items-center justify-center rounded-full sm:w-13 sm:h-13 lg:w-16 lg:h-16 mt-2 lg:mt-7 ml-0 mr-auto shadow-md lg:absolute top-[80%] left-20 "
      >
